...
def crop_recommendation():
    ...
    json input name :['n','p','k','ph','state','city','area','season']
    input example:
    {
    "state" : "gujarat",
    "city" : "ahmedabad",
    ...
    if request.method == "POST":
        ...
        season_le_path = os.path.join("static", "labelencoder", "season_le.joblib")
        season_le = joblib_load(season_le_path)
        model_season_encoded = int(season_le.transform([model_season])[0])
        model_para = [model_n, model_p, model_k, model_temp, model_humidity, model_ph, model_rainfall, model_area, model_season_encoded]


        # Use per-crop models for recommendation
        import glob
        crop_model_dir = os.path.join("static", "models")
        crop_model_files = glob.glob(os.path.join(crop_model_dir, "*.joblib"))
        crop_scores = []
        failed_models = []
        for model_path in crop_model_files:
            crop_name = os.path.splitext(os.path.basename(model_path))[0]
            try:
                model = load(model_path)
                # Predict yield/score for this crop
                score = model.predict([model_para])[0]
                crop_scores.append((crop_name, score))
            except Exception as e:
                failed_models.append((crop_name, str(e)))
                continue

        # Sort crops by predicted score (descending)
        sorted_list = sorted(crop_scores, key=lambda l: l[1], reverse=True)
        # Top 5 crops
        chart_list = sorted_list[:5]
        pie_chart_labels = [c[0] for c in chart_list]
        temp_values = [c[1] for c in chart_list]
        # Normalize scores to percentage
        total = sum(temp_values) if sum(temp_values) > 0 else 1
        pie_chart_values = [round((v / total) * 100, 2) for v in temp_values]

        # Error handling: if no crops predicted, return error
        if not pie_chart_values:
            error_message = "No crop models could make a prediction."
            debug_info = {"failed_models": failed_models}
...
def yield_prediction():
    ...
    if request.method == "POST":
        ...
        # Load local yield model
        model = load("static/models/yield_model.joblib")
        pred_yield = model.predict([model_para])[0]
